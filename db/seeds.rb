...
file_path = '/media/patanj/2CB2E365B2E331CE/dev/current/bulk_files/test-a4lh5i4opkvwv-20230517-20230517-1684651540110.xlsx'
# file_path = '/media/patanj/2CB2E365B2E331CE/dev/current/ressources/bulk_files/bulk-a2q8jkm889vce2-20230520-20230520-1684651565562.xlsx'
filename = file_path.split("/bulk_files/").last
taken_date = filename.split("-")[3]
fake_account = Account.all.sample

xlsx = Roo::Spreadsheet.open(file_path)

sheet = xlsx.sheet('Sponsored Products Campaigns')
sheet_header = sheet.row(1)
sheet_translated_header = sheet_header.to_h {|i| [i, nil]}.transform_keys!(CampaignImportRow::KEYS_PUBLIC_TRANSFORMER).keys

sheet.drop(1).each do |row|
  temp_attributes = [sheet_translated_header, row].transpose.to_h
  campaign_import_row_attributes = temp_attributes.merge(account_id: fake_account.id, taken_date: taken_date)

  CampaignIntegratorJob.perform_async(campaign_import_row_attributes.to_json)
end
